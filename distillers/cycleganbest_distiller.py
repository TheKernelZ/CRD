...
class CycleganBestDistiller(BaseCycleganBestDistiller):
    ...
    def backward_G_Student(self):
        ...
        s = self.Sfake.squeeze()
        t = teacher_image.squeeze()
        self.loss_G_row = self.rkd(s.transpose(0, 1), t.transpose(0, 1))
        self.loss_G_column = self.rkd(s.transpose(0, 2), t.transpose(0, 2))

        s_patch = F.unfold(self.Sfake, kernel_size=32, stride=32).permute(0, 2, 1).reshape(-1, 3, 32, 32)
        t_patch = F.unfold(teacher_image, kernel_size=32, stride=32).permute(0, 2, 1).reshape(-1, 3, 32, 32)
        self.loss_G_patch = self.rkd(s_patch, t_patch)

        self.set_requires_grad([self.netD_teacher_A], False)  # Ds require no gradients when optimizing Gs
        self.loss_G_D = self.criterionGAN(self.netD_teacher_A(self.Sfake), True)
        
        self.loss_G_student += 25 * self.loss_G_row                          # row-wise loss
        self.loss_G_student += 25 * self.loss_G_column                       # column-wise loss
        self.loss_G_student += 25 * self.loss_G_patch                        # patch-wise loss
        self.loss_G_student += self.loss_G_style + self.loss_G_feature  # perceptual loss
        self.loss_G_student += self.loss_G_D                            # online teacher discriminator
